...
import { ArrowLeft, CheckCircle, FileText, Info, UserCircle, XCircle } from 'lucide-react';
...
export default function VerifyApplicantPage() {
  ...
  const handleConfirmAction = () => {
    ...
    if (selectedAction === 'verify') {
      ...
      toastMessage = `Pendaftaran "${applicant.fullName}" telah ditolak.`;
    }
    
    // In a real app, you would send this update to the backend.
    // For this prototype, we just show a toast and navigate back.
    ...
    switch (selectedAction) {
      case 'verify':
        return {
          ...
          description: `Apakah Anda yakin ingin memverifikasi pendaftar atas nama ${applicant.fullName}?`,
          actionText: "Ya, Verifikasi"
        };
      case 'reject':
        return {
          title: "Konfirmasi Penolakan",
          description: `Apakah Anda yakin ingin menolak pendaftaran atas nama ${applicant.fullName}?`,
           actionText: "Ya, Tolak"
        };
    }
  }
      ...
      <div className="max-w-6xl mx-auto w-full">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <Button variant="destructive" className="flex-1 sm:flex-none" onClick={() => handleActionClick('reject')}>
              <XCircle className="mr-2 h-4 w-4"/>
              Tolak Berkas
            </Button>
            <Button className="flex-1 sm:flex-none" onClick={() => handleActionClick('verify')}>
          ...
          <div className="lg:col-span-2">
            <Card className="h-full">
              <CardHeader>
                ...
                <CardDescription>
                  Klik pada gambar untuk melihat versi lebih besar (simulasi).
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {applicantDetailsMock.documents.map(doc => (
                     <a href={doc.url} target="_blank" rel="noopener noreferrer" key={doc.id} className="group">
